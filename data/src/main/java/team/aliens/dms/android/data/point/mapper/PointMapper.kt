...
fun FetchPointsResponse.toModel(): PointStatus = PointStatus(
    totalPoints = this.totalPoint,
    points = this.pointResponses.toModel(),
)

fun List<FetchPointsResponse.PointResponse>.toModel(): List<Point> =
    this.map(FetchPointsResponse.PointResponse::toModel)

fun FetchPointsResponse.PointResponse.toModel(): Point = Point(
    id = this.id,
